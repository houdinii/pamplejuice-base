...
namespace houdinii
{
    ...
    void CompressorSection::updateTimeConstants()
    {
        ...
        attackCoeff = std::exp (-1.0f / (attackMs * 0.001f * sampleRate)); // NOLINT(*-narrowing-conversions)
        releaseCoeff = std::exp (-1.0f / (releaseMs * 0.001f * sampleRate)); // NOLINT(*-narrowing-conversions)
    }
